...
|---|---|---|---|---|
| 10/07/2026 (2) | Edge Function `rapatrier-page` DÉPLOYÉE par Olivier (CLI, option A) + M39 réel + correctif parseur (« 2/3 de c. à c. de cinq-parfums » : le « de » avant l'unité masquait l'unité) | 105 verts (test parseur ajouté), 1 todo | M39 ✓ en réel sur la base : « Poulet sauté à la citronnelle et au piment » (Marie Claire, capture Evernote lot 3+) importé de bout en bout — fiche pré-remplie fidèle (titre, source réutilisée, 4 pers., 13 ingrédients, étapes), catégorie « Plat » vidée à la relecture, enregistrement (128 recettes), même URL retentée → « déjà là » sans appel réseau ; Marmiton réel → « Aucune recette structurée » (leur JSON-LD n'est pas servi côté serveur : repli prévu, rien d'écrit) ; les 2 lignes mal structurées ont été corrigées PAR l'app (Modifier → Enregistrer re-parse) | question à Olivier : vider automatiquement la catégorie « Plat » à l'import (convention : vide = plat) ? ; leçon pilotage : après un rechargement HMR, `import('/src/lib/…')` sans horodatage Vite crée une 2e instance VIDE du store — lire le DOM, pas le module |
